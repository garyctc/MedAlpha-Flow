  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      ...
      <main className="p-6 space-y-8">
        <div>
           <span className="text-xs font-semibold text-primary uppercase tracking-wider mb-2 block">Step 2 of 3</span>
           <h2 className="text-xl font-semibold text-foreground mb-4 font-display">How long have you had these symptoms?</h2>
           ...
           <RadioGroup value={duration} onValueChange={handleDurationChange} className="space-y-3">
             {[
               ...
               <div key={opt.id} className={`flex items-center space-x-3 p-4 rounded-3xl border transition-all shadow-[var(--shadow-card)] ${duration === opt.id ? "bg-primary/10 border-primary" : "bg-card border-border"}`}>
                 <RadioGroupItem value={opt.id} id={opt.id} className="text-primary" />
                 <Label htmlFor={opt.id} className="flex-1 cursor-pointer font-medium text-foreground">{opt.label}</Label>
               </div>
             ))}
           </RadioGroup>
        </div>
        ...
        <div>
           <h2 className="text-xl font-semibold text-foreground mb-6 font-display">Rate your discomfort (1-10)</h2>
           <div className="bg-card p-6 rounded-3xl border border-border shadow-[var(--shadow-card)]">
             <div className="text-center text-4xl font-semibold text-primary mb-6">{painLevel[0]}</div>
             <Slider
               value={painLevel}
               ...
               className="mb-2"
             />
             <div className="flex justify-between text-xs text-muted-foreground font-semibold uppercase mt-4">
               <span>Mild</span>
               <span>Severe</span>
             </div>
           </div>
